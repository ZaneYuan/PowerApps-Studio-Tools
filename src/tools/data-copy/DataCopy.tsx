      ...
      {entityLogicalName && rows.length > 0 && (
        <>
          ...
          {writeResults && writeResults.length > 0 && (
            ...
            // as the (often very wide, unwrapped error text) table rows, which dragged 下载日志 off
            // to whatever the widest row's far edge was — invisible until scrolled all the way
            // right (see Bugs/8.24.md #5). Now the header is a fixed-width bar that never scrolls
            // horizontally at all, and only the table body underneath it does.
