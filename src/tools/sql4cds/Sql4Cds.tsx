...
export default function Sql4Cds() {
  ...
  useEffect(() => {
    setMatchInfo(null);
    setMatchError(null);
    if (result.kind !== "mutate" || !activeConnectionId || !entitySet || !entityLogicalName) return;
    if (mutateFilter === null && mutateFetchXml === null) return;
    let cancelled = false;
    const abort = new AbortController();
    setMatchLoading(true);
    fetchEntityMeta(activeConnectionId, entityLogicalName)
      .then((meta) => {
        const matching = mutateFetchXml
          ? queryMatchingIdsViaFetchXml(activeConnectionId, entitySet, meta.primaryIdAttribute, mutateFetchXml, abort.signal)
          : queryMatchingIds(activeConnectionId, entitySet, meta.primaryIdAttribute, mutateFilter!, abort.signal);
        return matching.then((m) => ({ ...m, primaryIdAttribute: meta.primaryIdAttribute }));
      })
      .then((m) => {
  ...
  }, [result.kind, activeConnectionId, entitySet, entityLogicalName, mutateFilter, mutateFetchXml]);
    ...
    <div className="max-w-none space-y-6">
      ...
      {result.kind === "mutate" && (
        <>
          ...
          {matchInfo && (
            <p className={`text-xs ${matchInfo.truncated ? "text-amber-600 dark:text-amber-400" : "text-gray-500 dark:text-gray-400"}`}>
              匹配到 {matchInfo.totalCount.toLocaleString()} 条记录{matchInfo.truncated ? "（数量过大，未能全部取回——请缩小 WHERE 范围）" : "，全部处理"}
            </p>
          )}
